const mongoose = require('mongoose');

const { Schema, model } = mongoose;

const AnimalSchema = new Schema({
  name: {
    type: String,
    trim: true,
  },
  animalType: {
    type: String,
    enum: ['cat', 'dog', 'bird', 'reptile', 'horse', 'other'],
    default: 'other',
  },
  breed: {
    type: String,
    default: 'unknown',
  },
  age: {
    type: String,
    default: 'unknown',
  },
  history: {
    type: String,
    required: true,
  },
  photo: {
    data: {
      type: String,
      default: '',
    },
    contentType: {
      type: String,
      enum: ['image/jpeg', 'image/png'],
      default: 'image/png',
    },
  },
});

module.exports = model('Animal', AnimalSchema);
